--Sp_GetFeeds 1, null, 'Diandra Soares'
CREATE proc [dbo].[Sp_GetFeeds]  
(
...
BEGIN  
	...
	,rtrim(ltrim(substring(ISNULL(Feed.Description, ''),0,150))) --adding case for description if blank
		+ CASE
	...
	Where ISNull(Feed.Active,1) = 1 AND 
	(@SearchText IS NULL OR (@SearchText IS Not Null AND ISNULL(Feed.Title, '') Like '%' + @SearchText + '%'))
	) AS Feed
