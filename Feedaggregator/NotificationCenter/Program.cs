    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            */
            ...
            push.RegisterGcmService(new GcmPushChannelSettings(Common.GoogleAPIKey));
            //Fluent construction of an Android GCM Notification
            //IMPORTANT: For Android you MUST use your own RegistrationId here that gets generated within your Android app itself!



            DataSet dsNotificationtosend = SqlHelper.ExecuteDataset(Common.BGCon, CommandType.StoredProcedure, "SP_GetDevicesforNotification");
            if (dsNotificationtosend != null && dsNotificationtosend.Tables.Count > 0 && dsNotificationtosend.Tables[0].Rows.Count > 0)
            {
                for (int i = 0; i < dsNotificationtosend.Tables[0].Rows.Count; i++)
                {
                    string strDeviceID = Convert.ToString(dsNotificationtosend.Tables[0].Rows[i]["DeviceID"]);
                    string strMessage = Convert.ToString(dsNotificationtosend.Tables[0].Rows[i]["NewNumber"]);
                    push.QueueNotification(new GcmNotification().ForDeviceRegistrationId(strDeviceID)
                                  .WithJson("{\"New Gupshup\":\" " + strMessage + "\"}"));
                }
            }
        ...
        static void NotificationSent(object sender, INotification notification)
        {
            ...
            if (sender.GetType().FullName == "PushSharp.Android.GcmPushService")
            {
                string registrationID = ((PushSharp.Android.GcmNotification)(notification)).RegistrationIds[0].ToString();
                SqlParameter spNotificationMessage, spDeviceID;
                spNotificationMessage = new SqlParameter("@NotificationLogMessage", "Sent: " + sender + " -> " + notification);
                spDeviceID = new SqlParameter("@DeviceID", registrationID);
                SqlParameter[] spArr = { spNotificationMessage, spDeviceID };
                SqlHelper.ExecuteNonQuery(Common.BGCon, CommandType.StoredProcedure, "Sp_InsertNotificationLog", spArr);
            }
        }
        ...
        static void NotificationFailed(object sender, INotification notification, Exception notificationFailureException)
        {
            ...
            if (sender.GetType().FullName == "PushSharp.Android.GcmPushService")
            {
                string registrationID = ((PushSharp.Android.GcmNotification)(notification)).RegistrationIds[0].ToString();
                SqlParameter spNotificationMessage, spDeviceID;
                spNotificationMessage = new SqlParameter("@NotificationLogMessage", "Failure: " + sender + " -> " + notificationFailureException.Message + " -> " + notification);
                spDeviceID = new SqlParameter("@DeviceID", registrationID);
                SqlParameter[] spArr = {spNotificationMessage,spDeviceID};
                SqlHelper.ExecuteNonQuery(Common.BGCon, CommandType.StoredProcedure, "Sp_InsertNotificationLog", spArr);
            }
